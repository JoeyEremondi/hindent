...
data Term1 a
  ...
  | Fun1 a
         a
  | EmptyRecord1 
  | Record1 (Map.Map String a)
            a


data TermN a
  = PlaceHolder String
  | AliasN Var.Canonical
           [(String,TermN a)]
           (TermN a)
  | VarN a
  | TermN (Term1 (TermN a))
    ...
    case content of
      ...
      Var flex maybeSuper Nothing -> 
        do  freshName <- getFreshName maybeSuper
            liftIO $
              UF.modifyDescriptor variable $
              \desc -> desc {_content = Var flex maybeSuper (Just freshName)}
            return (T.Var freshName)

      Alias name args realVariable -> 
        do  srcArgs <- 
              mapM (\(arg,tvar) -> (,) arg <$> variableToSrcType tvar) args
...
termToSrcType term =
    case term of
      ...
      Fun1 a b -> 
        T.Lambda <$> variableToSrcType a <*> variableToSrcType b
      ...
      Record1 fields extension -> 
        do  srcFields <- 
              Map.toList <$> Traverse.traverse variableToSrcType fields
