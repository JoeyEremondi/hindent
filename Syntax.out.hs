  ...
  case err of
    ...
    BadPattern name ->
      Report.report
        ...
        Nothing
        ("The free variable `" ++
         name ++ "` appears more than once in this pattern.")
        (Help.stack
           ...
           , Help.reflowParagraph $
             "In Elm, pattern matching works by binding these free variables to subsections\
                  ...
                  \ different subsections though! When you say `" ++
             name ++
             "` in some code, there\
    ...
    InfixDuplicate opName ->
      Report.report
        "INFIX OVERLAP"
        Nothing
        ("The infix declarations for " ++
         Help.functionName opName ++ " must be removed.")
        (Help.reflowParagraph $
         "The precedence and associativity of a particular operator can only be set in\
              \ one place in a code base, and " ++
         Help.functionName opName ++ " has already been set somewhere else.")
    ...
    TypeWithoutDefinition valueName ->
      Report.report
        ...
        ("There is a type annotation for `" ++
         valueName ++ "` but there" ++ " is no corresponding definition!")
        (text $
    ...
    UnusedTypeVarsInAlias typeName givenVars unused ->
      Report.report
        ...
        ("Type alias `" ++
         typeName ++
         "` cannot have unused type variables: " ++ Help.commaSep unused)
        (Help.stack
           ...
           , dullyellow $
             ...
             map
               ...
               ("type" :
                "alias" :
                typeName : filter (`notElem` unused) givenVars ++ ["=", "..."]) ])
    ...
    MessyTypeVarsInAlias typeName givenVars unused unbound ->
      Report.report
        ...
        Nothing
        ("Type alias `" ++
         typeName ++ "` has some problems with type variables.")
        (Help.stack
           [ Help.reflowParagraph $
           ...
           , dullyellow $
             ...
             map
               ...
               ("type" :
                "alias" :
                typeName :
                filter (`notElem` unused) givenVars ++ unbound ++ ["=", "..."]) ])
...
unboundTypeVars declKind typeName givenVars unboundVars@(tvar:_) =
  Report.report
    ...
    (Help.capitalize declKind ++
     " `" ++
     typeName ++
     "` must declare its use of type variable" ++ Help.commaSep unboundVars)
    (Help.stack
       [ text "You probably need to change the declaration like this:"
       , hsep $
         map text (declKind : typeName : givenVars) ++
         map (dullyellow . text) unboundVars ++ map text ["=", "..."]
       , Help.reflowParagraph $
         "Here's why. Imagine one `" ++
         typeName ++
         "` where `" ++
         tvar ++
         "` is an Int and\
        ...
        Parsec.Expect msg ->
          ...
          in 
            if msg == tab then
              ...
              hint
                { _expected = Set.insert msg' (_expected hint) }

        Parsec.Message msg ->
          hint
            { _messages = msg : _messages hint }
      ...
      case msgs of
        [msg] ->
          case extractSpecialMessage msg of
            Just (MsgKeyword kwd) ->
              ( "It looks like the keyword `" ++
                kwd ++ "` is being used as a variable."
        ...
        _ ->
          ("I ran into something unexpected when parsing your code!", Nothing)
